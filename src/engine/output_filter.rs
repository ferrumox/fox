...
pub(crate) struct PerRequestState {
    /// Reasoning-block delimiters for this model. Default `<think>`/`</think>`;
    /// set to the model's own markers (e.g. Gemma's `<|channel>`/`<channel|>`) so
    /// the filter separates reasoning from the answer regardless of format.
    pub(super) think_open: String,
    pub(super) think_close: String,
    /// True while we are inside a reasoning block.
    ...
    /// When true, prepend the reasoning-open tag to the very first emitted token.
    /// Set when `initial_in_thinking=true` so that clients see a well-formed
    /// reasoning block even though the opening tag was injected into the prompt
    /// rather than generated by the model.
...
impl Default for PerRequestState {
    fn default() -> Self {
        Self {
            ...
            model_control_patterns: Vec::new(),
            utf8_buf: Vec::new(),
            thinking_chars: 0,
            max_thinking_chars: 0,
            emit_think_open_tag: false,
        }
    }
}
...
pub(crate) fn apply_output_filter(state: &mut PerRequestState, raw: &str) -> (String, bool) {
    ...
    if state.emit_think_open_tag {
        ...
        if state.show_thinking {
            let open = state.think_open.clone();
    ...
    // 1. Enter reasoning block (open tag, e.g. `<think>` or Gemma's `<|channel>`).
    if raw.contains(&state.think_open) {
        state.in_thinking = true;
        state.thinking_chars = 0; // reset budget counter for each new block
        if state.show_thinking {
            // Emit the open tag so the user can see when reasoning starts.
            return (raw.to_string(), false);
        }
        return (String::new(), false);
    }

    // 2. Exit reasoning block; text *after* the closing tag goes to the pending buffer.
    if raw.contains(&state.think_close) {
        state.in_thinking = false;
        let close = state.think_close.clone();
        if state.show_thinking {
            ...
            // Any text after the close tag in the same token also needs to be flushed.
        ...
        if let Some(idx) = raw.find(&close) {
