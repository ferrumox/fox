...
impl InferenceEngine {
    ...
    pub(super) async fn handle_logits(
        ...
        'req: for (req_id, token_logits) in results {
            let req = running.iter().find(|r| r.id == *req_id);
            let Some(req) = req else {
                continue;
            };

            // A speculative decode step commits several tokens for one request; process
            // them in order and stop at the first that finishes the request.
            ...
            for logits in token_logits {
                ...
                let reached_max =
                    req.generated_tokens + emitted_this_step + 1 >= req.max_new_tokens;
                ...
                let logprob = if is_eos {
                    ...
                    req.sampling.logprobs.and_then(|top_n| {
                        self.compute_token_logprob(&logits.values, token_id, top_n)
                    })
                };
                ...
                if is_done {
                    ...
                    self.per_request_state.remove(req_id);
                    self.model.free_grammar(*req_id);
                    // This request is done — drop any remaining speculatively-committed tokens.
                    continue 'req;
                }
