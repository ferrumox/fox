...
impl McpServer {
    ...
    async fn tool_generate(
        ...
        let temperature = args
            .get("temperature")
            .and_then(|v| v.as_f64())
            .unwrap_or(crate::api::shared::sampling_defaults::TEMPERATURE as f64)
            as f32;
...
async fn run_inference(
    ...
    use crate::api::shared::sampling_defaults;
    ...
    engine
        .submit_request(req)
        .map_err(|e| (INTERNAL_ERROR, format!("request rejected: {e}")))?;
...
mod tests {
    ...
    /// A server over an empty registry in a temp models dir — enough for every
    /// protocol-level test; nothing here loads a real model.
    fn test_server(dir: &std::path::Path) -> McpServer {
        let mut cfg =
            crate::model_registry::RegistryConfig::embedded(dir.to_path_buf(), 4 * 1024 * 1024);
        cfg.max_context_len = Some(512);
        cfg.keep_alive_secs = 0;
        let registry = Arc::new(ModelRegistry::new(cfg, std::collections::HashMap::new()));
        McpServer::new(registry, dir.to_path_buf())
    }
